// Removes a contributor card from the archive by name or GitHub username.
...
//   node scripts/remove-card.js --name "Ada Lovelace" --dry-run
//   node scripts/remove-card.js --name "Ada Lovelace" --index 1   ← pick from multiple matches
//
// If multiple cards match, the script lists them with indices and exits.
// Re-run with --index N to select the one you want to remove.
//
...
if (!nameArg && !usernameArg) {
  console.error('Usage: node remove-card.js --name "Display Name" | --username github-handle [--index N] [--dry-run]')
  process.exit(1)
}
...
for (const file of archiveFiles) {
  ...
  entries.forEach((entry, index) => {
    const nameMatch = nameArg && entry.name.toLowerCase() === nameArg.toLowerCase()
    const usernameMatch =
      usernameArg &&
      entry.contacts.some(c => {
        ...
        return handle === target || link.endsWith(`/${target}`)
      })
...
if (matches.length > 1 && selectedIndex === null) {
  ...
  console.log(
    `\nExample: node scripts/remove-card.js ${nameArg ? `--name "${nameArg}"` : `--username "${usernameArg}"`} --index 0`
  )
  process.exit(1)
}
...
console.log(`✅ Removed "${entry.name}" from ${file} (${entries.length} entries remaining)`)
