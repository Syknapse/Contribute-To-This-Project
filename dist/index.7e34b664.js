...
const createArchiveObject = i1 => {
  const container = document.querySelector('.container')
  const archiveObject = document.createElement('object')
  archiveObject.setAttribute('id', `archiveObject_${i1}`)
  archiveObject.setAttribute('data', `archive/archive_${i1}.html`)
...
let current = 1
const getArchiveCards = i1 => {
  createArchiveObject(i1)
  document.getElementById(`archiveObject_${i1}`).onload = function() {
    const archiveObject = document.getElementById(`archiveObject_${i1}`)
...
$('#toggle-box-checkbox').on('change', function() {
  if (this.checked) $('body').addClass('night')
...
if (document.location.pathname.indexOf('fullcpgrid') > -1) demo()
...
function searchCard() {
  ...
  for (i = 0; i < cards.length; i++)
    if (!cards[i].textContent.toLowerCase().includes(input)) cards[i].style.display = 'none'
    else {
